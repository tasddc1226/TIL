...
## 4. YAGNI in Django
- YAGNI는 `You Aren't Gonna Need`라는 의미이다. 이 원칙은 필요 이상의 개발을 하지 말라는 것이다.
- 예를 들어 데코레이터에 대한 토론을 이전에 한 뒤, 아마 나중에 필요할 것 같은 내용들도 고려해서 그때 당시에 개발해둔 것이다.
- 즉, 지금 당장은 필요없지만 미래를 위한 작업을 해둔 뒤 시간이 흘러 결국 잊혀지게 될 수 있다는 것이다.
- 따라서 너무 미리 앞서서 나아가려고 하지 마라.